...
void *ptfsal_closeHandle_listener_thread(void *args)
{
  ...
  while (1) {
    ...
    if (msg_bytes != -1) {
      ...
      handle_index_to_close = ccl_find_oldest_handle();
      if (handle_index_to_close != -1) {
        ccl_update_handle_nfs_state (handle_index_to_close, CCL_CLOSE);
        close_rc = ccl_implicit_close_for_nfs(handle_index_to_close);
      }
